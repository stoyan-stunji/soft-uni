...
public:
    ...
    void setParent(const std::weak_ptr<FileSystemObject>& parent)
    {

    }

    std::weak_ptr<FileSystemObject> getParent() const 
    {
...
private:
    ...
    bool navigateSingleStep(std::string path)
    {
        if(path == "..")
        {
            ...
            if(parentPtr)
            {
                ...
                std::dynamic_pointer_cast<FileSystemObjectsContainer>(parentPtr);
                return true;
            }
            else
            {
    ...
    std::shared_ptr<FileSystemObject> findIn
    (std::shared_ptr<FileSystemObjectsContainer> dir, std::string name)
    {
        for(auto fso : *dir)
        {
            if(fso->getName() == name)
    ...
    {
        auto previousContainer =
        std::dynamic_pointer_cast<FileSystemObjectsContainer>(obj->getParent().lock());
